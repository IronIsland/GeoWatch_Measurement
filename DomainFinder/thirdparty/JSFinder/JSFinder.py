...
def find_by_url_deep(url,threads):
    urls = [url]
    visited_urls = set()
    with concurrent.futures.ThreadPoolExecutor(max_workers=threads) as executor:
        while urls:
            new_urls = []
            futures = [executor.submit(find_by_url, url) for url in urls]
            for future in concurrent.futures.as_completed(futures):
                result = future.result()
                if result:
                    new_urls.extend(result)
            visited_urls.update(urls)
            urls = [url for url in new_urls if url not in visited_urls]
    return visited_urls
...
if __name__ == "__main__":
	...
	if args.file == None:
		if args.deep is not True:
			...
			urls = find_by_url_deep(args.url,threads=threads)
			giveresult(urls, args.url)
	else:
